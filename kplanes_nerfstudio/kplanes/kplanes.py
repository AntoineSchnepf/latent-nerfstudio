...
class KPlanesModelConfig(ModelConfig):
    ...
    near_plane: float = 0.2 # changes related to latent_nerfstudio
    """How far along the ray to start sampling."""

    far_plane: float = 4.0 # changes related to latent_nerfstudio
...
class KPlanesModel(Model):
    ...
    def populate_modules(self):
        ...
        # renderers
        bg_color = self.config.background_color if self.latent_or_rgb == 'rgb' else self.latent_bg_color
        self.renderer_rgb = RGBRenderer(background_color=bg_color, latent_or_rgb=self.latent_or_rgb)
    ...
    def get_image_metrics_and_images(
        ...
        if self.latent_or_rgb == 'latent': # changes related to latent_nerfstudio, we don't compute LPIPS on latent images
            lpips = -1
        else:
