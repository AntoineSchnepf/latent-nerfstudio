...
class RGBFieldHead(FieldHead):
    ...
    def __init__(self, in_dim: Optional[int] = None, activation: Optional[nn.Module] = nn.Sigmoid(), out_dim: int = 3) -> None:
        super().__init__(in_dim=in_dim, out_dim=out_dim, field_head_name=FieldHeadNames.RGB, activation=activation) # changes related to latent_nerfstudio: out_dim is now a parameter
